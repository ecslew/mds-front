    ...
    <template v-else>
      <div class="container">
        ...
        <div class="row">
          <div class="col-sm-8">
            <p
              class="pic"
              :style="{background: 'rgba(0,0,0,0.05) url(' + programs.photos +')no-repeat center/cover'}"
            ></p>
            <div class="author-info clearfix">
              <div class="copy-link">
                ...
                >
                  <img src="static/img/icon/web_icon_link.png" height="48" />
                </p>
              ...
              <p class="avator">
                <img
                  :src="'https://api.medishares.net/apiTools/getAddressHead?address=' + programs.creator + '&v=1.0'"
                  width="20"
                />
              </p>
          ...
          <div class="col-sm-4">
            <div class="progress">
              <p
                class="progress-bar"
                :style="'width:'+parseFloat(programs.supportTotal/programs.amount*100)+'%'"
              ></p>
            </div>
            ...
            <div :class="[{unPay:programs.endDate<=0},'confirm supportBtn']" @click="payModal">
              <template v-if="programs.type==1">{{$t('payment')}}</template>
              <template v-else>{{$t('donate')}}</template>
            </div>
          </div>
        </div>
      </div>
      <section class="detail-info">
        ...
        <div class="container detail-info-container">
          <div class="row">
            <div class="col-sm-8">
              <div class="details" v-show="isActive" v-html="programs.des"></div>
...
export default {
  ...
  methods: {
                  ...
                  data: {
                    ...
                    quantity:
                      parseFloat(amount).toFixed(
                        _this.programs.targetTokenDecimal
                      ) +
                      " " +
                      _this.programs.targetToken,
                    memo:
                      '###{"ID":' +
                      _this.programs.eosID +
                      ',"creator":"' +
                      _this.programs.creator +
                      '","comment":"' +
                      note +
                      '"}###'
                  }
    ...
    getProjectInfo() {
      ...
      let url =
        _this.globalData.domain + "/apiCrowdfunding/getInfo?eosID=" + eosID;
      ...
      $.get(
        ...
        function(res) {
          if (res.success) {
            ...
            _this.programs.amount = parseFloat(res.data.amount).toFixed(
              _this.programs.targetTokenDecimal
            ); // 目标金额

            // transfer
            $.get(
              _this.globalData.domain + "/apiCrowdfunding/getTransRecord",
              {
                crowdfundingNo: _this.programs.crowdfundingNo
              },
              function(res) {
                // 支持金额和支持人数
                _this.programs.supportTotal = parseFloat(
                  res.data.total
                ).toFixed(_this.programs.targetTokenDecimal);
                ...
                if (res.success) {
                  ...
                  $.each(res.data.trans, function(index, event) {
                    _this.support.push({
                      ...
                      address: event.from,
                      amount:
                        parseFloat(event.amount).toFixed(
                          _this.programs.targetTokenDecimal
                        ) +
                        " " +
                        res.data.token, // 1.0000 EOS
    ...
    getEosPrice() {
      let _this = this;
      $.get(
        "https://api.coinmarketcap.com/v1/ticker/eos/?convert=USD",
        {},
        function(res) {
          _this.totalEosPriceUsd = parseFloat(
            res[0]["price_usd"] * _this.amount
          ).toFixed(2);
        },
        "json"
      );
